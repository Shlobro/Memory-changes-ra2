...
def safe_write_int(pm, address, value):
    try:
        pm.write_int(address, value)
    except Exception as e:
        print(f"Failed to write memory at {address}: {e}")


def read_game_data(pm):
    ...
    for i in range(MAXPLAYERS):
        ...
        if class_base_ptr != 0xffffffff and class_base_ptr != 0:
            ...
            player_data.append({
                ...
                "tanks": tanks,
                "buildings": buildings,
                "address": real_class_base + BALANCEOFFSET  # Store address for later use
            })

    return player_data


def display_table(data, pm):
    def update_balance(player_index):
        player = data[player_index]
        new_balance = simpledialog.askinteger("Update Balance", f"Enter new balance for {player['username']}:")
        if new_balance is not None:
            safe_write_int(pm, player["address"], new_balance)
            messagebox.showinfo("Success", f"Balance updated for {player['username']}")
    ...
    # Define columns
    columns = ["username", "balance", "spent_credit", "is_winner", "is_loser", "power_output", "power_drain", "country_name", "color", "color_scheme", "action"]
    ...
    for i, player in enumerate(data):
        tree.insert("", "end", values=(
            ...
            f"RGB({player['color'][0]}, {player['color'][1]}, {player['color'][2]})", player["color_scheme"], "Update Balance"
        ))
...
if __name__ == "__main__":
    while True:
        try:
            ...
            game_data = read_game_data(pm)
            if game_data:
                display_table(game_data, pm)
            else:
