...
def print_game_data(data):
    for i, player in enumerate(data):
        ...
        print(f"  Spent Credit: {player['spent_credit']}")
        print(f"  Winner: {player['is_winner']}")
        print(f"  Loser: {player['is_loser']}")
        print(f"  Power: {player['power_output']} / {player['power_drain']}")
        print(f"  Color: RGB({player['color'][0]}, {player['color'][1]}, {player['color'][2]})")
        print(f"  Color Scheme: {player['color_scheme']}")

        print("  Infantry:")
        for field, value in player['infantry']._fields_:
            if value != 0:
                print(f"    {field}: {getattr(player['infantry'], field)}")

        print("  Vehicles:")
        for field, value in player['tanks']._fields_:
            if value != 0:
                print(f"    {field}: {getattr(player['tanks'], field)}")

        print("  Buildings:")
        for field, value in player['buildings']._fields_:
            if value != 0:
                print(f"    {field}: {getattr(player['buildings'], field)}")
